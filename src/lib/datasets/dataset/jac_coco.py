...
from lib.datasets.dataset.utils import _bbox_overlaps, rotate_bbox
...
class JAC_COCO_36(data.Dataset):
    ...
    def run_eval_db_middle(self, results):
        ...
        for image_id, result in results.items():
            ...
            image_anno = self.coco.loadImgs(ids=[image_id])
            image_path = image_anno[0]['file_name']
            image_name = image_path.split('/')[-1]
            image = cv2.imread(os.path.join('Dataset/Jacquard/coco/512_cnt_angle/test',image_path))
            ...
            for i in range(len(boxes_s_pr)):
                ...
                if max_s < boxes_s_pr[i]:
                    ...
                    bbox_pr.append([x_c - w / 2, y_c - self.avg_h / 2,
                                    ...
                                    angle])
                # cv2.imwrite('/home/yusheng/code/GraspKpNet/src/result/'+ image_name, image)
            if len(bbox_pr) == 0:
                continue
